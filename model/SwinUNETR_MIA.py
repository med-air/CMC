...
class MIA_Module(nn.Module):
    ...
    def __init__(self, in_dim):
        ...
        self.conv1 = nn.Conv3d(in_channels=768, out_channels=14, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        img_F_mia_feat = self.conv1(img_F_mia)
        img_F_mia_fusion = F.interpolate(img_F_mia_feat, size=(96, 96, 96), mode='trilinear', align_corners=True)
...
class SwinUNETR(nn.Module):
    ...
    def forward(self, x_in):
        ...
        img_F_ds, img_F_mia, img_F_mia_fusion = self.MIA_module(dec4)

        dec3 = self.decoder5(img_F_mia, hidden_states_out[3])
        dec2 = self.decoder4(dec3, enc3)
        dec1 = self.decoder3(dec2, enc2)
        dec0 = self.decoder2(dec1, enc1)
        out = self.decoder1(dec0, enc0)
        logits = self.out(out)
        fused_outputs = torch.cat((img_F_ds, img_F_mia_fusion, logits), dim=1)  # 通道维度拼接
