...
class SwinUNETR(nn.Module):
    ...
    def forward(self, m1_in, m2_in):
        # print(x_in.shape, task_id.shape)
        # m1_in, m2_in = torch.chunk(x_inputs, 2, dim=1)  # 按通道拆分

        hidden_states_out_m1 = self.swinViT(m1_in, self.normalize)
        hidden_states_out_m2 = self.swinViT(m2_in, self.normalize)
        ## for CT data
        enc0 = self.encoder1(m1_in)
        enc1 = self.encoder2(hidden_states_out_m1[0])
        enc2 = self.encoder3(hidden_states_out_m1[1])
        enc3 = self.encoder4(hidden_states_out_m1[2])
        dec4 = self.encoder10(hidden_states_out_m1[4])
        m1_img_F_ds = dec4
        MIA_feat_m1 = self.MIA_module(m1_img_F_ds)

        ## for MRI data
        enc0_m2 = self.encoder1(m2_in)
        enc1_m2 = self.encoder2(hidden_states_out_m2[0])
        enc2_m2 = self.encoder3(hidden_states_out_m2[1])
        enc3_m2 = self.encoder4(hidden_states_out_m2[2])
        dec4_m2 = self.encoder10(hidden_states_out_m2[4])
        m2_img_F_ds = dec4_m2
        MIA_feat_m2 = self.MIA_module(m2_img_F_ds)
        fuse_feat = self.fusion_layer(MIA_feat_m1, MIA_feat_m2)
        MIA_m1 = fuse_feat + MIA_feat_m1
        MIA_m2 = fuse_feat + MIA_feat_m2
        MIA_feat_m1 = MIA_m1
        MIA_feat_m2 = MIA_m2
        ...
        ### for CT data
        dec3 = self.decoder5(MIA_feat_m1, hidden_states_out_m1[3])
        dec2 = self.decoder4(dec3, enc3)
        dec1 = self.decoder3(dec2, enc2)
        dec0 = self.decoder2(dec1, enc1)
        out_m1 = self.decoder1(dec0, enc0)

        ### for MRI data
        dec3_m2 = self.decoder5(MIA_feat_m2, hidden_states_out_m1[3])
        dec2_m2 = self.decoder4(dec3_m2, enc3_m2)
        dec1_m2 = self.decoder3(dec2_m2, enc2_m2)
        dec0_m2 = self.decoder2(dec1_m2, enc1_m2)
        out_m2 = self.decoder1(dec0_m2, enc0_m2)

        logits_m1 = self.out(out_m1)
        logits_m2 = self.out(out_m2)
        return MIA_m1, MIA_m2, logits_m1, logits_m2
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = Mlp(hidden_size=dim, mlp_dim=mlp_hidden_dim, act=am1layer, dropout_rate=drop, dropout_mode="swin")
